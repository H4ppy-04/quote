...
def pull(force=False, check_dev=True, verbose=False):
    ...
    if not force:
        ...
        __print(
            verbose, "Fetched any and all changes with response: {}".format(fetch_resp)
        )
        # reset
...
def push(
    ...
    try:
        ...
        # correct credentials but something else went wrong, most likely 2-factor auth
            ...
            __print(
                verbose,
                "Cannot gain access, authentication failed. This is often caused by 2-factor authentication",
            )
            raise ValueError(
                "Cannot gain access, authentication failed. This is often caused by 2-factor authentication"
            )
